...
_RE_SHA1=re.compile(r'^[0-9a-f]{40}$') # SHA1计算后的40位Hash字符串的匹配正则表达式

#----------------------------------基本功能---------------------------


# 用来验证用户的管理员身份，
# 如果没有登陆（不是用户）或用户没有管理员属性，直接返回提示无权限
...
async def cookie2user(cookie_str):
    ...
    # cookie_str是user2cookie函数的返回值，逆向还原user信息
    if not cookie_str:
...
@get("/signout")
def signout(request):
    # 请求头部的referer,表示从 哪里 链接到当前页面,即上一个页面
    # 用户登出时,实际转到了/signout路径下,因此为了使登出毫无违和感,跳回上一个转过来的页面
    referer = request.headers.get("Referer")
    # 若无前一个网址,可能是用户新打开了一个标签页,则登录后转到首页
    r = web.HTTPFound(referer or '/')
    # 设置cookie的最大存活时间为0来 删除cookie，这里的“-deleted-”只是提示，关键在max_age=0
...
# 管理：修改博客，这里是指点击“修改”按钮的意思
@get('/manage/blogs/edit')
def manage_edit_blog(*, id):
    return {
        '__template__': 'manage_blog_edit.html',
        'id': id,
        'action': '/api/blogs/%s' % id#将blog提交到/api/blogs/{id}这个接口API
    }
...
async def api_delete_comments(id,request):
    check_admin(request)#管理员才有权限删除
    c= await Comment.find(id)

    if c is None:
        raise APIResourceNotFoundError('Comment')
    await c.remove()
    return dict(id=id)#返回被删除的评论的id
